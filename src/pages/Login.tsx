...
const Login: React.FC = () => {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    const success = await sendCode(fullPhoneNumber);
    setIsLoading(false);
    
    if (success) {
      setStep('verification');
    } else {
      setError(language === 'zh' ? '验证码发送失败，请重试' : 'Failed to send verification code, please try again');
    }
  };
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    const fullPhoneNumber = `${selectedCountry.code}${phoneNumber.replace(/\D/g, '')}`;
    const success = await login(fullPhoneNumber, verificationCode);
    setIsLoading(false);
